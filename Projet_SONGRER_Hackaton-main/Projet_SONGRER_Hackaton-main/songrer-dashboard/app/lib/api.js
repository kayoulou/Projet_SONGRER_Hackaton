...
const ALLOWED_DASHBOARD_ROLES = new Set(["SUPER_ADMIN", "ADMIN", "NGO"]);
...
export const authService = {
  async login(email, password) {
    try {
      ...
      if (!ALLOWED_DASHBOARD_ROLES.has(data?.user?.role)) {
        throw new Error("Accès refusé : rôle non autorisé pour le tableau de bord.");
      }
      localStorage.setItem(AUTH_USER_KEY, JSON.stringify(data.user));
      return data.user;
    } catch (error) {
      const admin = ENABLE_DEMO_LOGIN ? DEFAULT_MOCK_DATA.admins.find((item) => item.email === email) : null;
      if (admin && password === process.env.NEXT_PUBLIC_DEMO_ADMIN_PASSWORD && ALLOWED_DASHBOARD_ROLES.has(admin.role)) {
  ...
  async getCurrentUser() {
    ...
    try {
      ...
      if (data?.user && ALLOWED_DASHBOARD_ROLES.has(data.user.role)) {
        localStorage.setItem(AUTH_USER_KEY, JSON.stringify(data.user));
        return data.user;
      }
    } catch {
